...
object OcrTransformer : InputMessageTransformer, KoinComponent {
    ...
    override suspend fun transform(
        ...
        // Tool attachments are observations for the model, not uploads to replace with OCR.
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                transformUploadedImagesForOcr(messages, ctx.model, ::performOcr)
            } finally {
...
internal suspend fun transformUploadedImagesForOcr(
    messages: List<UIMessage>,
    model: Model,
    recognize: suspend (UIMessagePart.Image) -> String,
): List<UIMessage> {
    if (Modality.IMAGE in model.inputModalities) return messages
    return messages.map { message ->
        message.copy(parts = message.parts.map { part ->
            if (part is UIMessagePart.Image && part.url.startsWith("file:")) {
                UIMessagePart.Text(recognize(part))
            } else {
                part
            }
        })
